...
The starting point was `nmon_prompt.md`, a structured specification that asked
...
Claude Code processed `nmon_prompt.md` and produced `nmonArchitecture.md`, a
...
combined with the `--read nmonArchitecture.md` flag. The architecture document
...
- Drop `--read nmonArchitecture.md` entirely.
...
The planning prompt (`nmon_prompt.md`) was explicit about every required output
...
Every non-obvious architectural choice in `nmonArchitecture.md` includes a
